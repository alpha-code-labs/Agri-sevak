...
from services.safety_filter import inject_rag_warnings, get_auditor_safety_instruction
...
""".strip()
                    ...
                    try:
                        ...
                        # Layer 1: scan evidence for banned chemicals, inject warnings
                        if rag_results:
                            rag_results = inject_rag_warnings(rag_results, crop)
...
""".strip()
            ...
            final_text = await _run_auditor_prompt(response_text, crop=crop, start_total=start_total)
...
async def _run_auditor_prompt(text: str, crop: str = "", start_total: float = None) -> str:
    try:
        # Layer 2: inject crop-specific banned chemicals into auditor prompt
        safety_block = get_auditor_safety_instruction(crop) if crop else ""
